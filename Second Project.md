...
```python
# Remove the Borough that has Not Assigned value and reset index
df_filtered = df[(df.Borough != "Not assigned")]
# Reset the index and create a copy of the new data frame
df_filtered_re = df_filtered.reset_index().copy()
# Filter only the 3 columns'Postcode','Borough','Neighbourhood'
df_filtered_af=df_filtered_re[['Postcode','Borough','Neighbourhood']]
df_filtered_af.head()
```
...
```python
# Group by'Postcode','Borough' and merge the value In the Neighbourhood
grouped_df= df_filtered_af.groupby(['Postcode','Borough']).Neighbourhood.agg([('Neighbourhood', ', '.join)])
# reset the index
...
```python
#create a function to change the value in Neighbourhood rows that has Not assigned
def f(row):
    ...
    return val
grouped_df['Neighbourhood'] = grouped_df.apply(f, axis=1)
grouped_df.head()
```
...
```python
# read the Location data sets
df_2= pd.read_csv('https://cocl.us/Geospatial_data')
```


```python
# Rename the "Postal Code" to "Postcode" in order to match with the Postcode column in the first data frame
df_2.rename(index=str, columns={"Postal Code": "Postcode"}, inplace=True)
df_2.head()
```
...
```python
#Merge the Location data with the postal Code Data sets
Finish_df = pd.merge(grouped_df, df_2, on='Postcode')
Finish_df.head()
```
